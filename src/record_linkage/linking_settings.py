...
To do own config, it should be of the form
{"settings":{splink_settings}, "deterministic_rules":{deterministic_rules}}
"""
...
DEFAULT_LINKING_CONFIG = {
    "DEFAULT_SETTINGS": {
        "unique_id_column_name": DEFAULT_UNIQUE_ID_COLUMN_NAME,
        "blocking_rules_to_generate_predictions": [
            "l.first = r.first and substr(l.last,1) = substr(r.last,1)",  # noqa
            "l.last = r.last",
            ...
            "substr(l.last,4) = substr(r.last,4) and l.postal_code = r.postal_code",  # noqa
        ],
        "comparisons": [
            {
                ...
                "comparison_levels": [
                    ...
                    {
                        "sql_condition": 'damerau_levenshtein("first_l", "first_r") <= 1',  # noqa
                ...
                "comparison_description": "Exact match vs. "
                + "First within levenshtein threshold 1 vs. "
                + "First within damerau-levenshtein threshold 1 vs. "
                + "First within jaro_winkler thresholds 0.9, 0.8 vs. "
                + "anything else",  # noqa
            },
            {
                ...
                "comparison_description": "Exact match vs. "
                + "Last within levenshtein threshold 1 vs. "
                + "Last within damerau-levenshtein threshold 1 vs. "
                + "Last within jaro_winkler thresholds 0.9, 0.8 vs. "
                + "anything else",
            },
            {
            ...
            },
            {
                ...
                "comparison_description": "Exact match vs. "
                + "Address_Line_1 within levenshtein threshold 3 vs. "
                + "anything else",
            },
            {
                ...
                "comparison_description": "Exact match vs. "
                + "Address_Line_2 within levenshtein threshold 2 vs. "
                + "anything else",  # noqa
            },
        ],
        "link_type": "link_only",
...
def create_settings(settings: dict[str, Any] = {}) -> dict[str, Any]:
    ...
    for setting in (
        ...
        if not settings.get(setting):
            settings[setting] = DEFAULT_LINKING_CONFIG["DEFAULT_SETTINGS"][setting]
